Social media pipeline for Dante (male Poodle) and Flora (female Maltipoo). 8 Claude Code skills that chain into an end-to-end workflow.
...
Zero npm dependencies — All scripts use Node built-in fetch, fs, path, crypto, parseArgs
ESM throughout — "type": "module" in package.json
--env-file=.env — Native Node 20.6+ env loading, no dotenv
Separate scripts per provider — Simpler than abstracting; SKILL.md handles orchestration
JSON state files — Appropriate for low volume (~few posts/week), easy to debug and git-track
User checkpoints in pipeline — Never auto-publish; every step needs explicit approval
Verification
Skill	Test
